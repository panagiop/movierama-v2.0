...
describe('[PUT] api/posts/:id/dislike/', () => {
    ...
    it(`(testUser1) should successfully dislike a post (published by testUser2)`, async () => {
        const result: VoteResult = await dislikePost({
            postId: newPost2.id,
            userId: newUser1.id
        });
    ...
    it(`(testUser1) should get an error trying to dislike an already disliked post (published by testUser2)`, async () => {
        const result: VoteResult = await dislikePost({
            postId: newPost2.id,
            userId: newUser1.id
        });
